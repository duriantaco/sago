...
- [Control-Plane Roadmap](#control-plane-roadmap)
- [How it works](#how-it-works)
- [Using with Codex](#using-with-codex)
- [Using with Claude Code](#using-with-claude-code)
- [Using with Cursor](#using-with-cursor)
- [Using with Aider](#using-with-aider)
- [Using with any other agent](#using-with-any-other-agent)
...
**Codex:**
- Read [`docs/CONTROL_PLANE_ROADMAP.md`](docs/CONTROL_PLANE_ROADMAP.md) for the active product direction
- Read [`docs/CODEX_WORKFLOW.md`](docs/CODEX_WORKFLOW.md) for the builder loop
- Use `sago next --json` for task pickup, `sago checkpoint --json` for progress, and `sago status --json` for state and recommendations
...
The active implementation plan for `sago` lives in
[`docs/CONTROL_PLANE_ROADMAP.md`](docs/CONTROL_PLANE_ROADMAP.md).

That roadmap is the source of truth for product direction:

- `sago` stays a control-plane tool, not a generic agent runtime
- Codex is the first executor to optimize for
- the next major investments are structured state, evidence-backed checkpoints, and real phase gates
- memory is intentionally narrow and deterministic, and comes later

If you are building on `sago` or using Codex to execute the roadmap, start there first.
...
Codex is the primary executor target for the current roadmap because it fits `sago`'s explicit file contract and JSON command surfaces well.

Before executing anything, have Codex read:
...
1. Run `sago next --json` to get the next actionable task or blocked state
2. Implement only that scoped task
3. Run the task's `<verify>` command
4. Run `sago checkpoint --json` to record the result
5. Run `sago status --json` to confirm state, blockers, and recommendations
6. Run `sago replan` when a phase completes or repeated failures suggest a plan change

Example Codex instruction:

> Read `docs/CONTROL_PLANE_ROADMAP.md` and `docs/CODEX_WORKFLOW.md` first. Treat the roadmap as the source of truth. Use `sago next --json` to pick work, run the task verify command, record progress with `sago checkpoint --json`, and only mark roadmap tasks complete when code, tests, and proof are present.
...
4. Run `sago next` or `sago next --json` to get the next task — it shows the task details, dependencies, and context
5. Run each task's `<verify>` command — it must exit 0 before moving on
6. Run `sago checkpoint <task_id>` or `sago checkpoint --json` after each task to record progress in STATE.md
7. Run `sago status --json` to inspect current state, blockers, and recommendations
8. Repeat from step 4 until all tasks are complete
...
For a Codex-first executor contract, see [`docs/CODEX_WORKFLOW.md`](docs/CODEX_WORKFLOW.md).
